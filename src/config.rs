    ...
    fn apply_custom_launcher_configs(&self, entries: &mut Vec<GameWithData>) {
        ...
        for (launcher_slug, conf) in self.launchers.iter() {
            ...
            let sources = match launcher_slug.as_str() {
                ...
                "heroic" => {
                    if !conf.extra_args.is_empty() {
                        warn!(
                            "Heroic Games Launcher uses xdg-open to launch games - you probably \
                            don't want apply extra arguments"
                        )
                    }
                    vec![
                        HeroicGamesAmazon,
                ...
                "itch" => {
                    if !conf.extra_args.is_empty() {
                        warn!(
                            "Each Itch entry is launched differently - you probably don't want to \
                            apply extra arguments to all entries"
                        );
                    }
                    vec![Itch]
                }
